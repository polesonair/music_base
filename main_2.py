...
name_max_time = connection.execute(f'SELECT track_name FROM Tracklist where time = {max_time[0]}').fetchone()
...
long_tracks = connection.execute('SELECT time, name_of_track FROM Tracklist WHERE time >= 210').fetchall()
...
sel = connection.execute("""SELECT name_of_track FROM Tracklist WHERE track_name iLIKE 'me %%'
